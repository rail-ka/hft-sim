...
stage1 будет принимать сообщения от producer и отправлять processor на основе конфига: если processor один, то ему, если несколько, то используем Детерминированный Роутинг ("Sticky Routing"). Если processor загружен, откладываем во внутреннюю очередь?
